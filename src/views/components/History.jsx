...
export default function History({ historyTabState, historyState, setInitialRequest }) {
  ...
  return (
    <>
      <section className={'z-40 h-screen w-screen fixed top-0 right-0 history_tab ' + clsx({ "_open" : open })}>
      <aside className={`z-10 absolute ${open ? 'left-0' : 'left-[-100%]'} sc_800:w-full min-w-[40%] h-screen max-h-screen overflow-auto border-r border-white/5 bg-[#29292a] px-2 transition-all duration-300`}>
         <div className='w-full bg-[#29292ab6] sticky top-0 z-20 py-2 px-1 flex justify-between items-center backdrop-blur-sm'>
            ...
            <div className='flex flex-col gap-2 p-1'>
              {history.length > 0 ? history.map((h, index) => (
                ...
                <div className='w-full flex flex-wrap items-center font-medium text-white text-sm text-center gap-1 justify-between focus:ring-0 cursor-pointer'>
                  <button onClick={() => setInitialRequest(h)} className='flex flex-wrap gap-1 flex-grow'>
                    <div className="md:w-20 w-full border rounded-lg px-4 py-2 border-white/5 bg-white/5 hover:bg-white/5 focus:ring-0 mb-2 md:mb-0">
                      { h.method }
                    </div>
                  ...
                  <Button
                      type="button"
                      className="md:ml-2 w-full md:w-auto border border-red-500/50 bg-red-800/20 hover:bg-red-800/50 px-4 py-2" title="Delete"
